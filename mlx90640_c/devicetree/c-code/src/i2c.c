...
/*
**	@brief		:	Initialize struct i2c_device with defualt value
**	#device	    :	struct i2c_device struct
*/
void i2c_init_device(struct i2c_device *device)
{
...
/*
**	@brief		:	Get struct i2c_device struct desc
**	#device	    :	struct i2c_device struct
**  #buf        :   Description message buffer
**  #size       :   #buf size
**	@return		:	return i2c device desc
*/
char *i2c_get_device_desc(const struct i2c_device *device, char *buf, size_t size)
{
    memset(buf, 0, size);
...
**
*/
ssize_t i2c_ioctl_read(const struct i2c_device *device, unsigned int iaddr, void *buf, size_t len)
{
    struct i2c_msg ioctl_msg[2];
    struct i2c_rdwr_ioctl_data ioctl_data;
    // unsigned char is used for byte arrays as it's guaranteed 8-bit, unsigned (0-255),
    // and compatible with I2C system calls expecting unsigned char * buffers.
    // uint8_t could be used but requires <stdint.h>, while unsigned char is standard C.
    ...
    if (device->iaddr_bytes) {
        ...
        printf("Address max length %d 0x%X %b %d\n", INT_ADDR_MAX_BYTES, iaddr, iaddr, device->iaddr_bytes);
        for(int i = 0; i < INT_ADDR_MAX_BYTES; i++) {
            printf("Addr[%d]: 0x%02X\n", i, addr[i]);
        }
        ...
        printf("flag: 0x%b 0x%b 0x%p\n", addr[0], addr[1], ioctl_msg[0].buf);
        printf("test: 0x%X %b\n", test, test);
    ...
    printf("Bus: %d; R/W %d; NMsg %d\n", device->bus, I2C_RDWR, ioctl_data.nmsgs);
    printf("First Msg len: %d; addr: 0x%02x; buf %zu; flags %hu\n", ioctl_data.msgs[0].len, ioctl_data.msgs[0].addr, sizeof(ioctl_data.msgs[0].buf) , ioctl_data.msgs[0].flags);
    printf("Second Msg len: %d; addr: 0x%02x; buf %p; flags %hu\n", ioctl_data.msgs[1].len, ioctl_data.msgs[1].addr, ioctl_data.msgs[1].buf, ioctl_data.msgs[1].flags);
...
ssize_t i2c_ioctl_write(const struct i2c_device *device, unsigned int iaddr, const void *buf, size_t len)
{
    ssize_t remain = len;
    size_t size = 0, cnt = 0;
    ...
    struct i2c_rdwr_ioctl_data ioctl_data;
    // unsigned char buffer for address and data in I2C ioctl write
    unsigned char tmp_buf[PAGE_MAX_BYTES + INT_ADDR_MAX_BYTES];

    while (remain > 0) {

        size = GET_WRITE_SIZE(iaddr % device->page_bytes, (unsigned int)remain, device->page_bytes);
...
**	#device	:	struct i2c_device struct, must call i2c_device_init first
...
*/
ssize_t i2c_read(const struct i2c_device *device, unsigned int iaddr, void *buf, size_t len)
{
    ssize_t cnt;
    // unsigned char array for byte-level I2C address transmission
    unsigned char addr[INT_ADDR_MAX_BYTES];
    unsigned char delay = GET_I2C_DELAY(device->delay);
    ...
    if ((unsigned int)write(device->bus, addr, device->iaddr_bytes) != device->iaddr_bytes) {
...
*/
ssize_t i2c_write(const struct i2c_device *device, unsigned int iaddr, const void *buf, size_t len)
{
    ssize_t remain = len;
    size_t cnt = 0, size = 0;
    const unsigned char *buffer = buf;
    unsigned char delay = GET_I2C_DELAY(device->delay);
    // unsigned char buffer for combining address and data bytes for I2C write
    unsigned char tmp_buf[PAGE_MAX_BYTES + INT_ADDR_MAX_BYTES];
...
**
**  The iaddr (unsigned int) must be converted to an unsigned char array because:
**  - I2C transmits data byte-by-byte; multi-byte addresses need to be split into individual bytes.
**  - System calls like ioctl and write require unsigned char * buffers for byte data.
**  - Ensures big-endian byte order for I2C protocol compliance.
*/
void i2c_iaddr_convert(unsigned int iaddr, unsigned int len, unsigned char *addr)
{
    ...
    // This loop copies the last 'len' bytes from the big-endian representation
    // of iaddr into the addr buffer in the correct order for I2C transmission
    // (MSB first). It extracts bytes from the end of the 4-byte big-endian array.
    while (i >= 0 && j >= 0) {
